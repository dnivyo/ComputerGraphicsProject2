package nz.aut.cg.group5.project2.objects;
...
public class ObjectLoader {
    ...
    private void ProcessfIntData(String sdata[]) {
        ...
        for (int loop = 1; loop < sdata.length; loop++) {
            String s = sdata[loop];
            String[] temp = s.split("/");
            vdata[loop-1] = Integer.parseInt(temp[0]); //always add vertex indices
            if (temp.length > 1) { // we have v and vt data
                vtdata[loop-1] = Integer.parseInt(temp[1]); // add in vt indices
            } else {
                vtdata[loop-1] = 0; // if no vt data is present fill in zeros
            }
            if (temp.length > 2) { // we have v, vt, and vn data
                vndata[loop-1] = Integer.parseInt(temp[2]); // add in vn indices
            } else {
